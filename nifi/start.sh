...
prop_replace 'nifi.flowcontroller.autoResumeState'          "${NIFI_AUTO_RESUME_STATE:-true}"
...
prop_replace 'nifi.queue.swap.threshold'                    "${NIFI_QUEUE_SWAP_THRESHOLD:-100000}"

prop_replace 'nifi.cluster.protocol.heartbeat.interval'     "${NIFI_HEARTBEAT_INTERVAL:-15 sec}"
prop_replace 'nifi.cluster.node.protocol.threads'           "${NIFI_NODE_PROTOCOL_THREADS:-50}"
prop_replace 'nifi.cluster.node.connection.timeout'         "${NIFI_NODE_CONNECTION_TIMEOUT:-60 sec}"
prop_replace 'nifi.cluster.node.read.timeout'               "${NIFI_NODE_READ_TIMEOUT:-60 sec}"

prop_replace 'nifi.zookeeper.connect.timeout'               "${NIFI_ZK_CONNECT_TIMEOUT:-60 sec}"
prop_replace 'nifi.zookeeper.session.timeout'               "${NIFI_ZK_SESSION_TIMEOUT:-60 sec}"
